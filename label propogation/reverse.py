...
import sys
fd2 = open("ans1.txt","rw+")
min_words_in_output_line = 3
#remove the previous value of the nodename_to_num and paste its new value
...
# changes that i have done to initliaze the value of nodename_to_num direct from input1.txt
# print(nodename_to_num)
f= open('dis.txt','r')
...
start = -1
st = " "
# final_list=init_list_of_objects(10)
# for n in lis:
# 	if n!=[]:
# 		print 
# print final_list

# //single line remove the duplicates
# demodata = set()
for n in lis:
	if n!=[]:
		if start == n[0]:
			st = st +" "+ str(no_to_nodename[n[1]])
            # demodata.add(no_to_nodename[n[1]])
		else:
			#sprint s
			# s = []
			if start!=-1:
				uniqueKeyword = set()
				st  = st  +" "+ str(no_to_nodename[start])
				parts = st.split()
				for p in parts:
					uniqueKeyword.add(p)
				if len(parts) > min_words_in_output_line:
					for word in uniqueKeyword:
						print (word,end=" ")
			...
			start = n[0]
			st = " "
			st = st +" "+ str(no_to_nodename[n[0]])
                        # print st
                        # print("what is this")
			
# print("dont know what is this for")
print (st)
# print("final")
